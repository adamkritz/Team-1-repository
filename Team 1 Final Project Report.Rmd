...
This model is outstanding, with low VIF values and an adjusted r-squared of `r summary(modelcaratccc)$adj.r.squared`. This is clearly the best model out of the bunch.
...
reg.best10 <- regsubsets(price~. , data = diamondsNO_train, nvmax = 10, nbest = 1, method = "exhaustive")  # leaps::regsubsets() - Model selection by exhaustive (default) search, forward or backward stepwise, or sequential replacement
...
Finally, let's test the accuracy of the best linear model on the test data.
...
I added in price here, as I thought it would make sense from a customer perspective seeing what factors affect diamond cut. Even if the workers don't know it, as a customer I can still see if they are correlated after the fact

I also split the data overall to 4:1, so that may affect results.
