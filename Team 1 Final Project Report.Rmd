...
The diamonds dataset is an extremely popular dataset, so there has been plenty of work done on it. Much of this work is done through machine learning or more advanced statistical techniques that are hard for the average person to understand. Our group specifically wanted to create an easy to understand R Markdown file that has clear results from our modelling so the average diamond buyer can understand it.
...
We had done a little bit of work on the linear model in the midterm, but we are now interested in improving the model by using categorical variables. We are also interested in using the model for predictions. The first two questions of the linear model are a refresher from the midterm, and the last three employ our new knowledge.

* Principal Component Regression (PCR) for price
    + What can PCR reveal about the data that linear modelling cannot?
    + How is PCR able to understand the known multicollinearity in the data?
    + What is the best model PCR creates?
    
PCR is a newer modelling technique we are interested in, and we hope it is able to work in tandem with the linear model to predict price. We also hope it can tackle the multicollinearity better than linear models.
...
* K-Nearest Neighbor (KNN) Classification for cut
    ...
    + How does changing K affect accuracy?
    + How does changing K affect other methods of evaluation?
    
KNN classifier is another new modelling technique that can be used to classify diamonds based on their cut.

For models that predicts price, we will be looking at it from a consumer perspective. However, for models that predict cut, we will be looking from the perspective of the workers when they are cutting diamonds. 
...
We will be uniformly removing all the outliers in the numerical variables in the dataset to avoid any modelling issues. We removed any outliers in continuous variables with a z-score of less then -3.5 or over 3.5. This removed any clear outliers from the data.
...
We will also be scaling the data for the PCR and KNN models, as they require scaling.
...
From here we can move onto answering the SMART questions.

## Linear Regression to predict price

### Single Variable Models

We started by creating one model for each numerical variable. All the models for this section will be created on the train data, and then tested on the test data once we find the best model. We will be using adjusted r-squared as our criteria for the models.
...
Here is a plot of the best single variable model, carat, predicting on the train set for now.

``` {r single var models 3}
loadPkg("modelr")
df.with.predfin.from.model = add_predictions(diamondsNO_train,modelcarat)
...
While this model has low VIF values, it has about the same adjusted r-squared as the carat model, so it is not much of an improvement.
...
Let's start this section with a simple carat + cut model. We are going to primarily use carat here as our continious variable, because it is was the best single variable model.
...
``` {r caratccc plot}
df.with.predfin.from.model <- add_predictions(diamondsNO_train,modelcaratccc)
head(df.with.predfin.from.model)
loadPkg("ggplot2")
...
Here we can see the best model's prediction on the train set, performing quite well overall.
...
Here we can the carat * clarity model. This model has extremely high VIF values, and does not beat the best model.
...
Usually, when one is building a linear model, they start with feature selection in order to try to come up with the best model to use. However, due to the limited number of variables, it was easier to start with a model and then use the feature selection to confirm our findings.
...
Let's test the accuracy of the C+C+C+C model on the test data. While the model was not trained on the train data, we can still remake the same model using the same formula on the test data. Let's start by remaking the C+C+C+C model on the test data to check its adjusted r-squared.
...
Lastly, let's look at one other metric of the linear model on the test data. The mean absolute error of the model is `r mean(abs(summary(modelcaratcccT)$residuals))`. This means that on average, the model misses the price by plus or minus `r mean(abs(summary(modelcaratcccT)$residuals))`. This may seem high, but it is largely due to a few outliers. This can be seen in the residual plot.
...
For PCR modeling, we first subset all the continuous variables from the original dataframe.
All the data is scaled and centered already, so we can perform PCR.

```{r, results='markup'}
diamondscont <- subset(diamondsNO_SC_train, select = c ("carat", "depth", "table", "price", "x", "y", "z"))
```

Now, we can obtain the Principal components of our desired model. 
```{r, results='markup'}
...
From the above Principal components we can see that PC1 is affected by x and carat the most. We can build the model with one or more Principal components depending on the variance in price explained. In our case, we are trying to eliminate all but one continuous variables since they are collinear. Let us run a principal regression model from the Pls library and see the results. By performing PCR we can tackle the problem of multi-collinearity. We need to keep in mind the RMSE values while evaluating the model since the VIF checks will show 1 or less than 1 for data that is scaled.
...
We can see from the results of the PCR model that with just one component we can explain around 79 % of variation in price and as the components increase, the variation in price explained increases very marginally so it would make sense to build a model with just one variable. We can also look at the validation plot and see that with just one component, the RMSE probability value drops to favorable levels(below 0.4) and with the addition of more variables, the RMSE probability value only decreases slightly. Due to these findings, it is more sensible to build a model with just one variable or component and achieve good results without bringing down the efficiency of the model.

If we wish to select one of the continuous variables since they are all collinear, we can choose the PC1 which is mostly affected by the carat of the diamond.
...
Let's now test the accuracy of the best linear model, the only carat model, from the PCR section on the test data. We again will remake the model on the test data.

```{r testing22, results = TRUE}
modelcaratT = lm(price ~ carat, data = diamondsNO_test)
tab_model(modelcaratT)

```

The model performs well again here when using the test data.

The adjusted r-squared is `r summary(modelcaratT)$adj.r.squared`, meaning that roughly 85% of the variation in price can be explained by our model.

```{r testing23}
df.with.predfin.from.model1 <- add_predictions(diamondsNO_test,modelcaratT)
head(df.with.predfin.from.model1)
loadPkg("ggplot2")
ggplot(df.with.predfin.from.model1,aes(price,pred))+geom_point(aes(price,pred), alpha =0.2)+geom_line(aes(pred), colour="red", size=1) + labs(title = "Carat Model Built on Test Data", x = "Price", y = "Prediction") + xlim(c(0, 20000))
```

Here we can see how well the model performs when predicting on the test data. It performs well, but misses some of the higher priced diamonds.

Let's again look at the mean absolute error of the model, which is `r mean(abs(summary(modelcaratT)$residuals))`. This means that on average, the model misses the price by plus or minus `r mean(abs(summary(modelcaratT)$residuals))`.


```{r test52}
res1 <- resid(modelcaratT)
qqnorm(res1, ann = FALSE)
qqline(res1)
title("Residual Plot 1", xlab="Price (z-score)", ylab="Residuals")
```
...
```{r test62}
plot(density(res1), ann = FALSE)
title("Residual Plot 2", xlab="Residuals", ylab="Density")
```

Here we can see that the model definitely struggles with higher prices, often being very far off.

For a single variable model, it performs quite well on the test data.
...
In this section, we will not be using price to guess diamond cuts, as when workers cut diamonds they do not know the price. All other variables will be included. 
...
library(MASS)
require(nnet)
diamondsNO_train1 <- subset(diamondsNO_train, select = c ("carat", "depth", "table", "x", "y", "z", "cut", "color", "clarity"))
diamondsNO_test1 <- subset(diamondsNO_test, select = c ("carat", "depth", "table", "x", "y", "z", "cut", "color", "clarity"))
```

```{r logisticReg2}
# Training the multinomial model
multinom_model <- multinom(cut ~ ., data = diamondsNO_train1)

# Checking the model
summary(multinom_model)

# Predicting the values for train dataset
classPredicted <- predict(multinom_model, newdata = diamondsNO_test1, "class")

# Confusion matrix
library(caret)
cmlogit=confusionMatrix(diamondsNO_test1$cut,classPredicted)
cmlogit
```
...
tune= tuneRF(diamondsNO_train1[,-7],diamondsNO_trainCutLabel, stepFactor = 0.5, plot=TRUE,
...
The chart above helps us choose the best `mtry` to get a minimal Out of Bag Error. Obviously, 4 is the best `mtry` with around 22% OOB error.


```{r rfc, results='markup'}

rf<- randomForest(cut~ .,data=diamondsNO_train1, mtry=4, ntree=75) #fitting RandomForest Classification
pred = predict(rf, newdata=diamondsNO_test1[,-7]) # Prediction on test set
cm=confusionMatrix(diamondsNO_test1$cut,pred) #confusion matrix
print(rf)
```

The random forest classifier has been modeled with mtry as 4 and ntree as 75.
...
```{r}
diamondsNO_train2= diamondsNO_train1[,c(1, 2,3,4,5, 6,7,8)]
diamondsNO_test2= diamondsNO_test1[,c(1, 2,3,4,5, 6,7,8)]

rf2<- randomForest(cut~ .,data=diamondsNO_train2,mtry=4, ntree=75) #fitting RandomForest Classification
pred2 = predict(rf2, newdata=diamondsNO_test2[,-7]) # Prediction on test set
...
barplot(height=as.matrix(check),beside=TRUE,names.arg = c("Accuracy","Sensitivity","Specificity","Precision"),legend.text = c("Logistic","RandomForest"),col=c("blue","green"))
...
From the result, we can see the precision, recall rate and F-1 score of each level. For fair, good, premium, and ideal levels, these three scores is all great. But for very good level, the recall rate is only 0.373, and F-1 score is only 0.453. We can check the confusion matrix. There are 758 diamonds which actually is in very good level are predicted to be premium level and 573 are predicted to be ideal level. So this model works no so good on very good level.  
...
### Were we successful in our modelling?

Overall, we were able to create models that successfully predict the target variables.

In the predicting price, it is clear that the numerical data can only go so far, largely due to the multicollinearity. Carat is easily the best single predictor model, so if customers are looking for a computationally easy model or a quick trick to figure our the price of a diamond, they should look at the carat. Beyond that, the categorical variables were definitely helpful in increasing the accuracy of the model, but only to an extent. The C+C+C+C model was the best overall, though it was not that much better than the single carat model. It was mainly better at predicting higher priced diamonds. PCR confirmed these suspicions, by showing the the strength of the model barely increases after one variable. 

In predicting cut, both the random forest model and the KNN model were successful. Both models had a high accuracy, near 80%, when it came to classifying diamonds. This beats a random model by a long shot, which would have a 20% chance of getting the cut of a diamond correct. It was interesting to see that both modelling techniques crushed it in specificity, or the true negative divided by the true negative and false positive. In the random forest model, it was also interesting to note that depth and table were the best at predicting cut, compared to predicting price where these two variables were nearly useless. 

### Future research

In the future, we would like to obtain a dataset with more information about diamonds. We believe there are still more diamond characteristics that can be used as predictors, so more variables would always be helpful, especially for random forest modelling. We also would want to use a dataset where there are not as many multicollinear variables among the diamonds, that way linear modelling and PCR could be more effective. A dataset with more observations could also be of use, and we could then see if the optimal K value changes for KNN. 

On top of dataset changes, we would be interested in looking into other modelling techniques not covered in this report, and see how well they are able to perform in predicting. Lastly, we would like to look into other types of gems, and see if our models are consistent across different types of gems or not.
