...
* Principal Component Regression (PCR) for price
    + Which of the following variables but one exhibiting multicollinearity can be eliminated for regression after PCA : x, y, z, table, carat and depth?
...
It is a must that the data is scaled and centered, before performing PCR.
...
```{r, results='markup'}
linear_model_pcr_1a = lm(price~ x + depth , data= diamondsNO_train)
...
Let's test the accuracy of the best linear model on the test data. Firstly, let's simple check the adjusted r-squared value of a linear model on the test data. 
...
The model performs just as well when using the test data as it did on the training data.
...
Lastly, let's look at one other metric of of the linear model on the test data. The mean absolute error of the model is `r mean(abs(summary(modelcaratT)$residuals))`. This means that on average, the model misses the price by plus or minus `r mean(abs(summary(modelcaratT)$residuals))`.
...
Here we can see again that the model performs well overall, but has quite a few outliers. 

The model performs well on test data overall.
