...
loadPkg("ggplot2")
ggplot(df.with.predfin.from.model,aes(price,pred))+geom_point(aes(price,pred), alpha =0.2)+geom_line(aes(pred), colour="red", size=1) + labs(title = "Carat Plot", x = "Price", y = "Prediction") + xlim(c(0, 20000))
...
```
Its either a worse predictor, or has higher vif values
...
All feature selection reflects our findings, and is all the same basically

## Suhas - PCR modeling to predict price



## Solomon - Logistic Regression to predict cut
