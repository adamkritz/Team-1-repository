---
title: "Team 1 Mid-term Project Code"
...
Our diamonds dataset comes from Kaggle. Our data was originally taken from Tiffany & Co.'s snapshot pricelist from 2017. Tiffany and Co. is a large American luxury jewelry corporation, so we felt their data would be very applicable to those looking to buy diamonds.
...
Overall, the dataset is relatively clean. However, there are just a still a few abnormalities in the data.

* There are outliers in many of the variables
...
These are the SMART Questions we will answer throughout the course of our research. We believe these questions will allow those at any level of statistical knowledge to understand how the different characteristics of diamonds interact.
...
Cut has a different spread compare to the other two variables. In this case, the highest quantity of diamonds, `r length(which(diamonds$cut =='Ideal'))`, are located at the top at the Ideal level. The least are located in the Fair level, only containing `r length(which(diamonds$cut =='Fair'))`. As the level of cut increases, so do the quantity of diamonds in that level.
...
Table is the first of two extremely centralized variables, as can be seen from the histogram. The data on the edges is so small, it cannot even be seen. The mean of table is `r mean(diamonds$table)` and the standard deviation is `r sd(diamonds$table)`, which is reflected in the histogram.
...
Carat has a much different spread than table and depth, though the values are still grouped together. However, this time they are near the bottom, as every value above 2 is considered an outlier for carat. Overall, the carat distribution is extremely right skewed. The mean for carat is `r mean(diamonds$carat)` and the standard deviation is `r sd(diamonds$carat)`.
...
## How Do Continuous Variables Affect Price? - Suhas
...
## How Do Categorical Variables Affect Cut? - Huang
...
This was probably the most interesting results we found. It seems that the diamonds in this dataset are statistically more expensive when they have lower quality cuts, colors, and clarity. This is opposite of what logic would tell us when looking at diamonds. There are a few possible explanations for this. One is that the diamonds in our dataset do not have good spreads of data for the their categorical variables, especially for something like cut. However, clarity and color both had enough diamonds in both their lower and higher levels. Another explanation is that bigger diamonds are likely to have lower clarity, cut, and color levels. This is because bigger diamonds are harder to craft to a fine level due to their size. This would explain why bigger, more expensive diamonds have lower other qualities. 

### How Do Continuous Variables Affect Price? 

Our results in this section were difficult to interpret. Since x, y, z, carat, table, and depth are all related to the size of a diamond, these variables had high levels of multicollinearity in models. This made it hard to create linear models using these variables. It was stil interesting to look at the results of the two-sample t-tests conducted on price here, and it seems that many of these variables do play a large factor in determining price.
...
In the future, it would be interesting to see how the how clarity, cut, and color change with the size of diamonds, to see if our theory on diamond sizes is correct. It would also be interesting to create a predictive model to guess the price or cut of a diamond when given certain other variables. This would allow people to customize a diamond to their own budget. 

It would also always be good to have more information on diamonds in the future, like the shape of the diamond. This could allow us to do more statistical analysis to see what factors influence diamonds. Lastly, it would be interesting to compare diamonds to other gems, like emeralds, sapphires, rubies, opals, pearls, etc. We are curious if the same factors that affect diamonds affect these gems as well.
