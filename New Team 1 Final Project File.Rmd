...
ggplot(cut_count, aes(x = "", y = n, fill = Cut)) +
  ...
  geom_text(size = 4, aes(label = paste0(round(n, 2)*100, "%")), position = position_stack(vjust = 0.5))+
...
```{r}
fit3a <- lm(price ~ x+y+z, data = diamonds)


xkabledply(fit3a, title = paste("Model :", format(formula(fit3a)) ) )
xkablevif(fit3a)
summary(fit3a)
...
```{r}
anova(fit1, fit2,fit3, fit4, fit5) -> anovaRes
anovaRes
str(anovaRes)
...
## What affects the cutting method? - Huang  
...
```{r}
loadPkg("ggstatsplot")
p = ggbarstats(data= diamonds, x = cut, y = color, bar.proptest = F, palette = 'Set3', results.subtitle = F,xlab = "Color" ,ylab = "Percentage" )
p
```
...
```{r}
p = ggbarstats(data= diamonds, x = cut, y = clarity, bar.proptest = F, palette = 'Set3', results.subtitle = F,xlab = "Clarity" ,ylab = "Percentage" )
p
```
